...
# Try to import the export helper so the server can regenerate the JSON export when needed
try:
...
def parse_sheet(book, target, strings):
    ...
    for raw in raw_rows[1:]:
        ...
        if any(item.values()):
            rows.append(item)
    # remove columns that have no non-empty values across all rows
    filtered_columns = []
    ...
    if len(filtered_columns) != len(columns):
        filtered_rows = [{column: row.get(column, "") for column in filtered_columns} for row in rows]
        return {"columns": filtered_columns, "rows": filtered_rows}
    return {"columns": columns, "rows": rows}
...
function renderSheets(sheets) {
  const values = ["all"].concat(sheets.map((sheet) => sheet.name));
  ...
  sheetPills.innerHTML = ['<button type="button" data-sheet="all" class="' + (activeSheet === "all" ? "active" : "") + '">All sheets</button>']
    .concat(sheets.map((sheet) => '<button type="button" data-sheet="' + escapeHtml(sheet.name) + '" class="' + (activeSheet === sheet.name ? "active" : "") + '">' + escapeHtml(sheet.name) + " (" + Number(sheet.count).toLocaleString() + ")</button>"))
    .join("");
...
function renderTable(data) {
  ...
  tbody.innerHTML = rows.length
    ...
    : '<tr><td class="empty" colspan="' + Math.max(columns.length, 1) + '">No matching records found.</td></tr>';

  resultCount.textContent = String(Math.min(rows.length, 1000));
  totalCount.textContent = String(data.total_matches || 0);
  fileName.textContent = data.file || "Unknown workbook";
}

async function runSearch() {
  const params = new URLSearchParams({ q: searchInput.value, sheet: activeSheet });
  statusText.textContent = "Searching records...";
  try {
    ...
    renderTable(data);
    statusText.textContent = data.result_count ? "Loaded workbook records" : "No matching records";
  } catch (error) {
...
def main():
    ensure_files()
    # Ensure the JSON export exists on startup. This helps if users open the static
    # page (which reads data/psa-data.json) or when the export is stale/missing.
    try:
        if export_workbook:
            export_workbook()
    except Exception:
        # Do not prevent the server from starting if export fails; errors are logged elsewhere.
        pass
